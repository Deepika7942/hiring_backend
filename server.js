...
// ✅ Fetch Applications by Status (Accepted/Rejected)
app.get("/api/applications/:status", async (req, res) => {
  try {
    ...
    const applications = await Application.find({ status: status.charAt(0).toUpperCase() + status.slice(1) });
    res.json(applications);
  } catch (error) {
    console.error("❌ Error fetching applications:", error);
    res.status(500).json({ message: "❌ Server error, unable to fetch data." });
...
app.put("/api/applications/:id/status", async (req, res) => {
  try {
    ...
    res.json({ message: `✅ Application updated to ${status}`, application: updatedApplication });
  } catch (error) {
    console.error("❌ Error updating status:", error);
    res.status(500).json({ message: "❌ Server error, unable to update status." });
  }
